...
void main() {
	...
	if (window) {
		...
		// Declaration 
		Rabbit* rabbits = new Rabbit[500];
		Fox* foxs = new Fox[50];
		init_rabbit_fox(&rabbits, 500, &foxs, 50);
		Rabbit* device_rabbits = nullptr;
		Fox* device_foxs = nullptr;
		SendToGPU((void**)&device_rabbits, rabbits, 500 * sizeof(Rabbit));
		SendToGPU((void**)&device_foxs, foxs, 50 * sizeof(Fox));
		// end Declaration
		...
		while (!glfwWindowShouldClose(window)) {
			...
			DrawFoxs(surface_map, device_foxs, kWidth, kHeight, static_cast<float>(current_frame) * 0.01f);
			DrawRabbits(surface_map, device_rabbits, kWidth, kHeight, static_cast<float>(current_frame) * 0.01f);
			move(surface_map, device_foxs, device_rabbits, kWidth, kHeight, static_cast<float>(current_frame) * 0.01f);

			char buffer[64];
			int ret = snprintf(buffer, sizeof buffer, "%f", static_cast<float>(current_frame) * 0.01f);
